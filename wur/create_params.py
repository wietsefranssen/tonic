...
soil_dict = soil(workPath + 'vic_params_global_0.5deg/global_soil_param_new', c=Cols(nlayers=root_zones))

# Read the snow parameters
snow_dict = snow(workPath + 'vic_params_global_0.5deg/global_snowbands_new', soil_dict, c=Cols(snow_bands=5))

# Read the veg parameter file
veg_dict = veg(workPath + 'vic_params_global_0.5deg/global_veg_param_new', soil_dict, lai_index=True, veg_classes=n_veg_classes)

# Read the veg library file
veg_lib = veg_class(workPath + 'vic_params_global_0.5deg/world_veg_lib.txt', skiprows=2)

# Determine the grid shape
#target_grid, target_attrs = calc_grid(soil_dict['lats'], soil_dict['lons'])
target_grid, target_attrs = read_netcdf(workPath + 'vic_params_global_0.5deg/mask_NL.nc')
...
write_netcdf(workPath + 'vic_params_global_0.5deg/params.vic5_NL.nc', target_attrs,
